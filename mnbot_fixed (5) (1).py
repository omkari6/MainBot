...
class DCMTradingBot:
    ...
    def start_new_series(self, side: str, current_price: float):
        ...
        try:
            ...
            logger.info(f"Started new {side} series - Size: {self.position.total_size:.6f}, Avg Price: {self.position.avg_price:.6f}, Total Position: {self.position.total_size:.6f}")
    ...
    def manage_existing_position(self, current_price: float, ma: float, upper_dcm: float, lower_dcm: float):
        ...
        if should_accumulate:
            try:
                ...
                logger.info(f"Accumulated {side}: +{new_quantity:.6f} at {current_price:.6f}, Avg Price: {self.position.avg_price:.6f}, Total Position: {self.position.total_size:.6f}")
