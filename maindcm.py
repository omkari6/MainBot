...
class DCMTradingBot:
    ...
    def place_market_order(self, side: str, quantity_usdt: float) -> Optional[Dict]:
        """Place market order in USDT terms"""
        try:
            ...
            logger.info(f"Market order placed: {side} {quantity_usdt} USDT of {self.config.pair}")
            return order
        except Exception as e:
    ...
    def manage_existing_position(self, close_price: float, upper_dcm: float, lower_dcm: float):
        ...
        try:
            ...
            # Check for consecutive DCM breaks (before maturation)
